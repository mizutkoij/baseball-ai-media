...
export async function generateMetadata({
  ...
  const title = `NPB リーダーズ（${year}年） — Baseball AI Media`;
  const url = `https://baseball-ai-media.vercel.app/rankings?year=${year}`;
  const description = `${year}年のNPBリーダーズ。wRC+・OPS+・ERA-・FIP-などの主要指標TOP20を網羅。PF補正の解説や選手比較への導線も完備。`;
...
export default async function Page({ searchParams }: { searchParams: { year?: string } }) {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-8">
      <JsonLd data={{ "@context":"https://schema.org","@type":"BreadcrumbList", itemListElement: breadcrumb }} />
      <JsonLd data={{ "@context":"https://schema.org","@type":"CollectionPage", name:`NPB リーダーズ ${year}` }} />
      ...
      <header className="mb-8">
        <div className="flex items-center justify-between gap-3 mb-6">
          ...
          <div className="flex items-center gap-3">
            <ShareButton 
              url={`https://baseball-ai-media.vercel.app/rankings?year=${year}`}
              title={`NPB リーダーズ ${year}`}
              text={`${year}年のNPB主要指標ランキング`}
              size="sm"
            />
          </div>
        </div>
      ...
      <section className="bg-slate-50 rounded-xl p-6">
        ...
        <div className="mt-4 pt-4 border-t border-slate-200">
          <p className="text-sm text-slate-600">
            ...
            、個別推移は{" "}
            <Link href="/compare/players" className="text-blue-600 hover:text-blue-800 underline">
              選手比較
            </Link>
            {" "}を参照してください。
          </p>
